...
parser.add_argument('--outfile', type=str, default="RedCDR_ccle", help="output file")
...
parser.add_argument('--epochs', type=int, default=500, help="the epochs for model")
...
parser.add_argument('--layers', type=int, default=1, help="Layers of gnn in attribute branch")
parser.add_argument('--rd', type=float, default=0.5, help="Coefficient of representation distillation loss")
parser.add_argument('--pd', type=float, default=1, help="Coefficient of prediction distillation loss")
...
drug_feature, mutation_feature, gexpr_feature, methylation_feature, data_new, nb_celllines, nb_drugs = LoadData_ccle(Drug_feature_file, Genomic_mutation_file, Gene_expression_file, Methylation_file, Cancer_response_exp_file)
# pickle.dump([drug_feature, mutation_feature, gexpr_feature, methylation_feature, data_new, nb_celllines, nb_drugs],open("data/ccle.data","wb"))
# drug_feature, mutation_feature, gexpr_feature, methylation_feature, data_new, nb_celllines, nb_drugs = pickle.load(open("data/ccle.data","rb"))
...
args_list = 'ep' + str(args.epochs) + '_' + 'lr' + str(args.lr) + '_' + 'drop' + str(args.dropout) + '_' + 'numk' \
            + str(args.numk) + '_' + 'rd' + str(args.rd) + '_' + 'pd' + str(args.pd) + '_' + 'dim' + str(args.dim_feat) + '_' + 'ratio' + str(args.test_ratio) + '_' + 'alpha' + str(args.alpha)+ '_' + 'layers' + str(args.layers)
print(args_list)
...
for args.seed in range(2023, 2033):
    ...
    model = RedCDR(atom_shape, [256, 256, 256], gexpr_feature.shape[-1], methylation_feature.shape[-1], args.dim_feat,
                   100, nb_celllines, nb_drugs, args.numk, args.layers, args.dropout, args.alpha, False, False, False,
                   0.1, device).to(device)
